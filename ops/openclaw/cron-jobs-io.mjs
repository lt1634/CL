...
export function withGatewayStopped(fn) {
  const hasPlist = fs.existsSync(GATEWAY_PLIST);
  if (hasPlist) {
    console.log("Stopping OpenClaw gateway for cron edit...");
    spawnSync("launchctl", ["unload", GATEWAY_PLIST], { stdio: "ignore" });
    sleep((parseInt(process.env.OPENCLAW_CRON_GRACE_STOP || "2", 10) || 2) * 1000);
  }
  try {
    return fn();
  } finally {
    if (hasPlist) {
      ...
      spawnSync("launchctl", ["load", GATEWAY_PLIST], { stdio: "inherit" });
    }
  }
}
